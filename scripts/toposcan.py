...
def scan_line(
    ...
    try:
        ...
        aotask = create_task()
        add_global_channel(aotask,CURR_CONTROL)
        framp = 0
        print('ramping down...')
        while True:
            print(', '.join(map('{:.3f}'.format,(framp,fmin))))
            framp = max(fmin,framp)
            write_sample(aotask,framp)
            if framp == fmin:
                break
            framp -= dframp
        if w is not None:
            wtarget = w-deltaf/2/30e3
            while True:
                print('target w:',wtarget)
                dwthresh = 0.0005
                wo = 0
                wthresh = 2500
                while True:
                    wp = tc.get_wavenumber_act()
                    print('wp',wp)
                    if wp < wthresh:
                        continue
                    dw = abs(wp-wo)
                ...
                print('dfp',dfp)
                dfpthresh = 50.0 # MHz
                if abs(dfp) < dfpthresh:
                    print('dfp below threshold. continuing to scan.')
                    break
                dfdi = -1500 # MHz / ma
                di = dfp / dfdi
                print('di',di)
                io = topo.get_diode_current()
                ip = io + di
                print('ip',ip)
                if ip < 94.0 or ip > 96.0:
                    raise ModeHopError('requested current too high. quitting.')
                topo.set_diode_current(io+di)            
        ...
        cotask = create_task()
        add_global_channel(cotask,TRIGGER)
        ...
        f = fmin
        df = scanrate / samprate
        nout = 0
        N = int((fmax - fmin)/scanrate*samprate)

        def update_buff(nout,f,started):
            ngen = get_samps_generated(aotask) if started else 0
            dn = nout - ngen
            n = chunksize - dn
            if n:
                data = np.minimum(f + df * np.arange(n),fmax)
                write_to_buff(
                    aotask,
                    np.minimum(f + df * np.arange(n),fmax)
                )
            return nout + n, min(f + df * n,fmax)

        nout, f = update_buff(nout,f,False)
        ...
        m = 0
        while nin < N:
            nout, f = update_buff(nout,f,True)
        ...
        print('ramping home...')
        while True:
            print(', '.join(map('{:.3f}'.format,(framp,0))))
            framp = max(0.0,framp)
            write_sample(aotask,framp)
            if framp == 0.0:
                break
...
def scan_and_save(
    deltaf,df,fscanrate,inchans,folder,name,
        notes=None,user_cb=_do_nothing,wait=wait,w=None
):
    ...
    scanparams = {
        ...
        'lockin phase':(lockin_phase,'degrees'),
...
def get_lock(wnum,n,epsilon_wnum,dt,req_stable):
    ...
    while True:
        ...
        if req_stable and not topo.get_diode_temperature_ready():
...
def _set_topo(wnum,f,*args):
    ...
    try:
        ...
        get_lock(wnum,n,epsilon_wnum,dt,False)
        ...
        get_lock(wnum,n,epsilon_wnum,dt,True)
